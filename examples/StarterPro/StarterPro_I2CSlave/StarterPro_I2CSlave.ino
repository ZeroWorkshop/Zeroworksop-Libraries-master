...
//  Serial.println(val.longVal[0]);
  ...
  switch(cmd){
    ...
    case CMD_RESET:
      ...
      break;
      case CMD_SET_MOTOR:
      MotorControl(slot,(uint8_t)val.longVal[0]);
      break;
...
void StepperContorl(uint8_t slot,long whatSpeed,long steps_to_move)
{
 ...
 */    
  ...
  for(int j = 0; j<abs(steps_to_move); j++)
  {
    for(int i = 0; i<50; i++)
    {
      if(direction == 1)
